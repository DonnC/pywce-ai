# Hooks
pywce hooks takes advantage of python reflection api where it uses a dotted path to the function to call.

Every hook defined must have take a `HookArg` parameter. This will be passed automatically by the engine when calling the hook.

Every hook will and must always return a HookArg instance, usually the passed arg.

Inside the `HookArg` model, there is a `session_manager` attribute. This is an implementation of `ISessionManager`

A developer can implement their own session manager from this instance. The ISessionManager is defined as below
